...
import { isBookingEnabled, RoomClosure, ROOM_COUNT } from "../../lib/booking";
...
export async function GET() {
  ...
  return NextResponse.json({ bookingOpen: isBookingEnabled() && settings.bookingOpen !== false, openRooms: settings.openRooms || Array.from({ length: ROOM_COUNT }, (_, index) => index + 1), closures: settings.closures || [] });
}
...
export async function POST(request: NextRequest) {
  ...
  return NextResponse.json({ success: true, bookingOpen: isBookingEnabled() && body.bookingOpen, openRooms, closures });
}
